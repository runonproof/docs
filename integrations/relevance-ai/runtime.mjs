...
export const CAPABILITIES = Object.freeze({
  company_check: Object.freeze({ id: "gb.solution.company_check.v1", path: "/v1/gb/solutions/company-check", method: "POST", price_usdc: "0.04", availability: "ACTIVE", purchasable: true, intent: "Verify identity and legal company state" }),
  supplier_approval: Object.freeze({ id: "gb.solution.supplier_approval.v1", path: "/v1/gb/solutions/supplier-approval", method: "POST", price_usdc: "0.20", availability: "ACTIVE", purchasable: true, intent: "Approve a supplier using company and UK sanctions evidence" }),
  restricted_party_screen: Object.freeze({ id: "restricted_business_party.screen.v1", path: "/v1/restricted-parties/screen", method: "POST", price_usdc: null, availability: "ACTIVE", purchasable: false, intent: "Screen a named party against the UK Sanctions List" }),
  replay: Object.freeze({ id: "uk.execution.replay.v1", path: "/v1/gb/executions/{execution_id}", method: "GET", price_usdc: "0.00", availability: "ACTIVE", purchasable: false, intent: "Replay one committed execution without source calls or a new charge" })
});

const CONDITIONED = Object.freeze({ bank_account_ownership: "gb.bank_account_ownership.verify.v1", tax_registration: "gb.tax_registration.verify.v1", business_licence: "gb.business_licence.verify.v1" });
...
export function selectCapability({ intent, country = "GB", has_execution_id = false }) {
  ...
  if (Object.hasOwn(CONDITIONED, intent)) return blocked("CAPABILITY_CONDITIONED_NON_PURCHASABLE", `${CONDITIONED[intent]} is not purchasable; stop and request qualified external evidence.`);
...
export function buildRequest({ capability, company_number, execution_id, idempotency_key, body = {} }) {
  ...
  return { method: definition.method, url: `${RUNONPROOF_ORIGIN}${definition.path}`, headers: { Accept: "application/json", "Content-Type": "application/json", "Idempotency-Key": idempotency_key }, body: payload, economic_intent: definition.purchasable };
}
...
export function normalizeDecision(value) {
  ...
  if (value.status === "PAYMENT_REQUIRED" || value.http_status === 402) return Object.freeze({ recommendation: "REVIEW", automatic_continuation: false, payment_required: true, reason_codes: ["EXPLICIT_PAYMENT_AUTHORIZATION_REQUIRED"], original: redact(value) });
  ...
  return Object.freeze({ recommendation, automatic_continuation: recommendation === "PROCEED", payment_required: false, decision: value.decision, authorization: value.authorization, machine_action: value.machine_action, reason_codes: [...value.reason_codes], coverage: value.coverage, source_health: value.source_health, evidence: value.evidence, limitations: value.limitations, valid_as_of: value.valid_as_of, valid_until: value.valid_until, execution_id: value.execution_id ?? null, replayed: value.replayed === true, economic_activity: value.economic_activity ?? { charged: false, quote_created: false, settlement_created: false } });
}
